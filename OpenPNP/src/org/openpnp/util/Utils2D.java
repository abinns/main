...
public class Utils2D
{
	 ...
	 * "actual" Locations that have been offset and rotated, calculate the angle
	 * of rotation and offset between them. Angle is the difference between the
	 * angles between the two ideal Locations and the two actual Locations.
	 * Offset is the difference between one of the ideal Locations having been
	 * rotated by Angle and the matching actual Location.
	 * 
	 * @deprecated (2016/01/30) Please see calculateAngleAndOffset2. This
	 *             function is no longer used in the core codebase, but it's
	 *             being left here in case other users have incorporated it into
	 *             their changes. It may be removed in the future.
	 * @param idealA
	 * @param idealB
	 * @param actualA
	 * @param actualB
	...
	public static Location calculateAngleAndOffset(Location idealA, Location idealB, Location actualA, Location actualB)
	{
		...
		double angle = Math.toDegrees(Math.atan2(actualA.getY() - actualB.getY(), actualA.getX() - actualB.getX()) - Math.atan2(idealA.getY() - idealB.getY(), idealA.getX() - idealB.getX()));
	...
	/**
	 * Given two "ideal" Locations and two matching "actual" Locations calculate
	 * the difference in rotation and offset between them. Angle is the
	 * difference in angle between the line through the two ideal Locations and
	 * the line through the two actual locations. Offset is the difference in
	 * position of the first ideal and first actual Location. This function
	 * differs from calculateAngleAndOffset in that it expects the ideal and
	 * actual locations to be close to each other, and instead of returning the
	 * total offset and angle this function only returns the difference between
	 * the ideal and actual. This function is intended to be used with the
	 * fiducial checker and has been tested with it. The function above used to
	 * be used for the fidicual checker but did not handle bottom coordinates
	 * correctly and it's still not clear why.
	 * 
	 * @param idealA
	 * @param idealB
	 * @param actualA
	...
	{
		return Utils2D.calculateBoardPlacementLocation(bl.getLocation(), bl.getSide(), bl.getBoard().getDimensions().getX(), placementLocation);
	}
	...
	{
		return Utils2D.calculateBoardPlacementLocationInverse(boardLocation.getLocation(), boardLocation.getSide(), boardLocation.getBoard().getDimensions().getX(), placementLocation);
	}
	...
	{
		point = Utils2D.translatePoint(point, center.getX() * -1, center.getY() * -1);
		point = Utils2D.rotatePoint(point, c);
		point = Utils2D.translatePoint(point, center.getX(), center.getY());
		point = Utils2D.translatePoint(point, x, y);
	...
	public static Point rotateTranslateScalePoint(Point point, double c, double x, double y, double scaleX, double scaleY)
	{
		point = Utils2D.rotatePoint(point, c);
		point = Utils2D.translatePoint(point, x, y);
		point = Utils2D.scalePoint(point, scaleX, scaleY);
		return point;
	}
