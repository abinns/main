...
{
	/**
	 * Given pixel coordinates within the frame of the Camera's image, get the
	 * offsets from Camera center to the coordinates in Camera space and units.
	 * The resulting value is the distance the Camera can be moved to be
	 * centered over the pixel coordinates. Example: If the x, y coordinates
	 * describe a position above and to the left of the center of the camera the
	 * offsets will be -,+. If the coordinates position are below and to the
	 * right of center the offsets will be +, -. Calling
	 * camera.getLocation().add(getPixelCenterOffsets(...) will give you the
	 * location of x, y with respect to the center of the camera.
	 * 
	 * @param camera
	 * @param x
	 * @param y
	 ...
	 * of the given camera. This is a helper method that simply adds the offsets
	 * from {@link VisionUtils#getPixelCenterOffsets(Camera, double, double)} to
	...
	{
		return camera.getLocation().add(VisionUtils.getPixelCenterOffsets(camera, x, y));
	}
