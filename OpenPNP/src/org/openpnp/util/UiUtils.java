...
{
	/**
	 * Functional interface for a Runnable that can throw an Exception but
	 * returns no value. Splits the difference between Runnable and Callable.
	 */
	...
	/**
	 * Functional wrapper for actions that may throw an Exception. Presents an
	 * error box to the user with the Exception contents if one is thrown.
	...
	/**
	 * Wrapper for submitMachineTask(Callable, Consumer, Consumer) which ignores
	 * the return value in onSuccess and shows a MessageBox when an Exception is
	 * thrown. Handy for simple tasks that don't care about the return value but
	 * want to notify the user in case of failure. Ideal for running Machine
	 * tasks from ActionListeners.
	 * 
	 * @param callable
	 * @return
	 */
	...
	{
		return UiUtils.submitUiMachineTask(callable, (result) -> {
		} , (t) -> {
	 ...
	 * @return
	 */
	public static <T> Future<T> submitUiMachineTask(final Callable<T> callable, final Consumer<T> onSuccess, final Consumer<Throwable> onFailure)
	{
