...
public class OpenCvUtils
{
	...
	/**
	 * Finds circles of the given min and maxDiameter, no less than minDistance
	 * apart by capturing an image from the given Camera. Results are returned
	 * as a List of Location where X and Y represent the center of the circle, Z
	 * is that of the Camera and rotation is the diameter of the circle found.
	 * Results are returned sorted by distance from the Camera Location in
	 * ascending order.
	 * 
	 * @param camera
	 * @param minDiameter
	 ...
	 * @throws Exception
	 */
	public static List<Location> houghCircles(Camera camera, Length minDiameter, Length maxDiameter, Length minDistance) throws Exception
	{
		OpenCvUtils.logger.debug("houghCircles({}, {}, {}, {})", new Object[]
		{ camera.getName(), minDiameter, maxDiameter, minDistance });
		...
		Mat circles = OpenCvUtils.houghCircles(mat, minDiameterPixels, maxDiameterPixels, minDistancePixels);
	...
	{
		OpenCvUtils.logger.debug("houghCircles(Mat, {}, {}, {})", new Object[]
		{ minDiameter, maxDiameter, minDistance });

		OpenCvUtils.saveDebugImage("houghCircles_in", mat);

		// save a copy of the image for debugging
		Mat debug = mat.clone();

		// hough requires grayscale images
		mat = OpenCvUtils.toGray(mat);

		// and prefers a blurred image
		mat = OpenCvUtils.gaussianBlur(mat, 9);

		// run the houghcircles algorithm
		Mat circles = new Mat();
		Imgproc.HoughCircles(mat, circles, Imgproc.CV_HOUGH_GRADIENT, 1, minDistance, 80, 10, (int) (minDiameter / 2), (int) (maxDiameter / 2));

		if (OpenCvUtils.logger.isDebugEnabled())
		{
			OpenCvUtils.drawCircles(debug, circles);
			OpenCvUtils.saveDebugImage("houghCircles_debug", debug);
		}

		OpenCvUtils.saveDebugImage("houghCircles_out", mat);
	...
	public static void saveDebugImage(String name, Mat mat)
	{
		if (OpenCvUtils.logger.isDebugEnabled())
			try
			{
				...
				File file = Configuration.get().createResourceFile(OpenCvUtils.class, name + "_", ".png");
	...
	{
		Imgproc.adaptiveThreshold(mat, mat, 255, Imgproc.ADAPTIVE_THRESH_MEAN_C, invert ? Imgproc.THRESH_BINARY_INV : Imgproc.THRESH_BINARY, 3, 5);
		return mat;
	}

	public static Mat thresholdOtsu(Mat mat, boolean invert)
	{
		Imgproc.threshold(mat, mat, 0, 255, (invert ? Imgproc.THRESH_BINARY_INV : Imgproc.THRESH_BINARY) | Imgproc.THRESH_OTSU);
		return mat;
	}
	...
	/**
	 * Convert the given Mat to grayscale. Conversion is done in place and if
	 * the Mat is already grayscale nothing is done.
	 * 
	 * @param mat
	 * @return
	 */
