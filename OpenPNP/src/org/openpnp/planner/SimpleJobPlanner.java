...
public class SimpleJobPlanner extends AbstractJobPlanner
{
	static class WeightedPlacementSolution extends PlacementSolution
	{
		...
		public WeightedPlacementSolution(Placement placement, BoardLocation boardLocation, Head head, Nozzle nozzle, NozzleTip nozzleTip, Feeder feeder)
		{
	...
	/**
	 * For N nozzles, create a list of every possible remaining solutions along
	 * with a weight. Weight is a cost of 0 or more with things like a nozzle
	 * change increasing the weight. Once all solutions are identified. Sort
	 * each list by weight and take the N lowest weighted solutions that do not
	 * conflict with each other. TODO: Is there a situation where the order in
	 * which we take the weighted solutions from their lists would cause
	 * successive nozzles to have to use less optimal solutions? I feel like
	 * this is a possible issue but I haven't been able to come up with an
	 * example case.
	 */
	@Override
	public synchronized Set<PlacementSolution> getNextPlacementSolutions(Head head)
	{
		...
		for (Nozzle nozzle : head.getNozzles())
			for (WeightedPlacementSolution solution : this.getWeightedSolutions(machine, nozzle))
				if (this.solutions.contains(solution.originalSolution))
				{
					results.add(solution);
					this.solutions.remove(solution.originalSolution);
					break;
				}
		...
		if (results.size() == 0 && this.solutions.size() > 0)
	...
	protected List<WeightedPlacementSolution> getWeightedSolutions(Machine machine, Nozzle nozzle)
	{
		...
		for (PlacementSolution solution : this.solutions)
		{
			Part part = solution.placement.getPart();
			if (part == null)
				continue;
			Set<NozzleTip> compatibleNozzleTips = SimpleJobPlanner.getCompatibleNozzleTips(nozzle, part);
			Set<Feeder> compatibleFeeders = SimpleJobPlanner.getCompatibleFeeders(machine, nozzle, part);
			for (NozzleTip nozzleTip : compatibleNozzleTips)
				for (Feeder feeder : compatibleFeeders)
				{
					WeightedPlacementSolution weightedSolution = new WeightedPlacementSolution(solution.placement, solution.boardLocation, nozzle.getHead(), nozzle, nozzleTip, feeder);
		...
		Collections.sort(weightedSolutions, SimpleJobPlanner.weightComparator);
	...
	public void setJob(Job job)
	{
		...
		for (BoardLocation boardLocation : job.getBoardLocations())
		{
			...
			for (Placement placement : boardLocation.getBoard().getPlacements())
			{
				...
				this.solutions.add(new PlacementSolution(placement, boardLocation, null, null, null, null));
			}
		}

		SimpleJobPlanner.logger.debug("Planned {} solutions", this.solutions.size());
	}
}
