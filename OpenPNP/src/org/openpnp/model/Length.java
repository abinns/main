...
public class Length
{
	...
	{
		return Length.parse(s, false);
	}

	/**
	 * Takes a value in the format of a double followed by any number of spaces
	 * followed by the shortName of a LengthUnit value and returns the value as
	 * a Length object. Returns null if the value could not be parsed.
	 */
	 ...
	 * system default units. If the Location itself does not have units set, the
	 * Location's units are set to the Length's units. Finally, if the
	...
	{
		return Length.setLocationField(configuration, location, length, field, false);
	}
	...
	{
		return new Length(this.value + d, this.units);
	}

	public Length add(Length length)
	{
		length = length.convertToUnits(this.units);
		return new Length(this.value + length.getValue(), this.units);
	}
	...
	{
		return String.format(Locale.US, "%2.3f%s", this.value, this.units.getShortName());
	}
	...
	public String toString(String fmt)
	{
		...
		return String.format(Locale.US, fmt, this.value, this.units.getShortName());
	}
