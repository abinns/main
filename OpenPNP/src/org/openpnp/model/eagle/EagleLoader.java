...
public class EagleLoader
{
	...
	public EagleLoader(InputStream in) throws Exception
	{
		...
		xmlreader.setFeature(EagleLoader.FEATURE_NAMESPACES, true);
		xmlreader.setFeature(EagleLoader.FEATURE_NAMESPACE_PREFIXES, true);
		...
		this.eagle = (Eagle) unmarshaller.unmarshal(source); // TODO change
															 // later to Eagle
															 // eagle =
		// (Eagle) unmarshaller.unmarshal(source);
		Drawing drawing = (Drawing) this.eagle.getCompatibilityOrDrawing().get(0);

		// Now see what we have
		try
		{
			this.layers = drawing.getLayers();
		...
		try
		{
			this.board = (Board) drawing.getLibraryOrSchematicOrBoard().get(0);
			// Now we need to extract the details
		} catch (Exception e)
		{
		...
		try
		{
			this.schematic = (Schematic) drawing.getLibraryOrSchematicOrBoard().get(0);
		} catch (Exception e)
		{
		...
		try
		{
			this.library = (Library) drawing.getLibraryOrSchematicOrBoard().get(0);

			Packages packages = this.library.getPackages();
