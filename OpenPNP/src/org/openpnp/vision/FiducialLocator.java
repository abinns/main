...
public class FiducialLocator
{
	...
	/**
	 * Create a template image based on a Placement's footprint. The image will
	 * be scaled to match the dimensions of the current camera.
	 * 
	 * @param fid
	 * @return
	 */
	private static BufferedImage createTemplate(Location unitsPerPixel, Footprint footprint) throws Exception
	{
		...
		// recognition performance because it allows some border around the
		// edges.
	 ...
	 * using the vision system. The function first moves the camera to the ideal
	 * location of the fiducial based on the board location. It then performs a
	 * template match against a template generated from the fiducial's
	 * footprint. These steps are performed thrice to "home in" on the fiducial.
	 * Finally, the location is returned. If the fiducial was not able to be
	 * located with any degree of certainty the function returns null.
	 * 
	 * @param fid
	 * @return
	 * @throws Exception
	...
	private static Location getFiducialLocation(BoardLocation boardLocation, Placement fid) throws Exception
	{
		...
		FiducialLocator.logger.debug("Locating {}", fid.getId());
		...
		// Create the template
		BufferedImage template = FiducialLocator.createTemplate(camera.getUnitsPerPixel(), fid.getPart().getPackage().getFootprint());
		...
		FiducialLocator.logger.debug("Looking for {} at {}", fid.getId(), location);
		...
		for (int i = 0; i < 3; i++)
		{
			...
			location = FiducialLocator.getBestTemplateMatch(camera, template);
			if (location == null)
			{
				FiducialLocator.logger.debug("No matches found!");
				return null;
			}
			FiducialLocator.logger.debug("{} located at {}", fid.getId(), location);
	...
	{
		// Create the template
		BufferedImage template = FiducialLocator.createTemplate(camera.getUnitsPerPixel(), footprint);

		// Wait for camera to settle
		Thread.sleep(camera.getSettleTimeMs());
		// Perform vision operation
		return FiducialLocator.getBestTemplateMatch(camera, template);
	}
	...
	public static Location locateBoard(BoardLocation boardLocation) throws Exception
	{
		...
		IdentifiableList<Placement> fiducials = FiducialLocator.getFiducials(boardLocation);
		...
		List<Placement> mostDistant = FiducialLocator.getMostDistantPlacements(fiducials);

		Placement placementA = mostDistant.get(0);
		Placement placementB = mostDistant.get(1);

		FiducialLocator.logger.debug("Chose {} and {}", placementA.getId(), placementB.getId());

		// Run the fiducial check on each and get their actual locations
		Location actualLocationA = FiducialLocator.getFiducialLocation(boardLocation, placementA);
		if (actualLocationA == null)
			throw new Exception("Unable to locate first fiducial.");
		Location actualLocationB = FiducialLocator.getFiducialLocation(boardLocation, placementB);
		...
		Location location = Utils2D.calculateAngleAndOffset2(idealLocationA, idealLocationB, actualLocationA, actualLocationB);
