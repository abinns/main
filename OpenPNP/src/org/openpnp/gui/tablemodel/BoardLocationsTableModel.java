...
public class BoardLocationsTableModel extends AbstractTableModel
{
	...
	{ "Board", "Width", "Height", "Side", "X", "Y", "Z", "ø", "Enabled?", "Check Fids?" };

	private Class[] columnTypes = new Class[]
	{ String.class, LengthCellValue.class, LengthCellValue.class, Side.class, LengthCellValue.class, LengthCellValue.class, LengthCellValue.class, String.class, Boolean.class, Boolean.class };
	...
	public int getRowCount()
	{
		if (this.job == null)
			return 0;
		return this.job.getBoardLocations().size();
	}

	@Override
	public Object getValueAt(int row, int col)
	{
		BoardLocation boardLocation = this.job.getBoardLocations().get(row);
		...
		switch (col)
		{
			...
			case 7:
				return String.format(Locale.US, this.configuration.getLengthDisplayFormat(), loc.getRotation(), "");
			case 8:
	...
	public void setValueAt(Object aValue, int rowIndex, int columnIndex)
	{
		try
		{
			BoardLocation boardLocation = this.job.getBoardLocations().get(rowIndex);
			if (columnIndex == 0)
			...
			else if (columnIndex == 1)
			{
				...
				location = Length.setLocationField(this.configuration, location, length, Length.Field.X);
			...
			} else if (columnIndex == 2)
			{
				...
				Length length = value.getLength();
				Location location = boardLocation.getBoard().getDimensions();
				location = Length.setLocationField(this.configuration, location, length, Length.Field.Y);
				boardLocation.getBoard().setDimensions(location);
			...
			} else if (columnIndex == 6)
			{
				...
				Length length = value.getLength();
				Location location = boardLocation.getLocation();
				location = Length.setLocationField(this.configuration, location, length, Length.Field.Z);
				boardLocation.setLocation(location);
