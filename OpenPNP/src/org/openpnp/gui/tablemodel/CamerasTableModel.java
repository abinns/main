...
public class CamerasTableModel extends AbstractTableModel
{
	...
	public CamerasTableModel(Configuration configuration)
	{
		...
		Configuration.get().addListener(new ConfigurationListener.Adapter()
		{
			...
			{
				CamerasTableModel.this.refresh();
			}
		});
	}
	...
	{
		Camera camera = this.cameras.get(row);
	...
	public void refresh()
	{
		this.cameras = new ArrayList<>(Configuration.get().getMachine().getCameras());
		for (Head head : Configuration.get().getMachine().getHeads())
			this.cameras.addAll(head.getCameras());
	...
	public void setValueAt(Object aValue, int rowIndex, int columnIndex)
	{
		try
		{
			Camera camera = this.cameras.get(rowIndex);
			if (columnIndex == 0)
