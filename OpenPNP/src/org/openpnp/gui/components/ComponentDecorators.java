...
public class ComponentDecorators
{
	...
	{
		ComponentDecorators.decorateWithAutoSelect(textField);
		ComponentDecorators.decorateWithLengthConversion(textField);
	}

	/**
	 * Adds a length conversion decoration to the JTextField. When the
	 * JTextField loses focus or has it's action triggered the text will be
	 * converted to a Length value in the system units and then have it's text
	...
	public static void decorateWithLengthConversion(JTextField textField)
	{
		textField.addActionListener(new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent event)
			{
				ComponentDecorators.convertLength((JTextField) event.getSource());
			}
		});
		textField.addFocusListener(new FocusAdapter()
		{
