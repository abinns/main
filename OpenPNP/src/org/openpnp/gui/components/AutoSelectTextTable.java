...
/**
 * From http://tips4java.wordpress.com/2008/10/20/table-select-all-editor/ The
 * RXTable provides some extensions to the default JTable 1) Select All editing
 * - when a text related cell is placed in editing mode the text is selected.
 * Controlled by invoking a "setSelectAll..." method. 2) reorderColumns - static
 * convenience method for reodering table columns
 */
...
public class AutoSelectTextTable extends JTable
{
	...
	{
		this.setSelectAllForMouseEvent(isSelectAllForEdit);
		this.setSelectAllForActionEvent(isSelectAllForEdit);
		this.setSelectAllForKeyEvent(isSelectAllForEdit);
	}
