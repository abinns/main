...
public class FiducialTest
{
	...
	static public Point fiducial(Mat gray, int min, int max) throws Exception
	{
		...
		FiducialTest.showResult("Canny", bw);
		// scan blobs
		...
		for (int i = contours.size(); i-- > 0;)
		{
			...
			if (f <= 0.8 || f >= 1.2 || t >= 1.2 || t <= 0.8 || box.get(i).size.width < min || box.get(i).size.width > max || a < 100.)
			{
				contours.remove(i);
				continue;
			}
		...
		if (contours.size() != 0)
		{
			...
			Imgproc.drawContours(FiducialTest.dst, contours, i, new Scalar(0, 0, 255), 2); // java
			// thing
			Core.circle(FiducialTest.dst, ret, 4, new Scalar(0, 255, 255), -1);
			Core.circle(FiducialTest.dst, new Point(r.x + r.width / 2, r.y + r.height / 2), 4, new Scalar(0, 0, 255), -1);
			FiducialTest.showResult("found", FiducialTest.dst);
			// <<<<<<<<<<< end of debug
	...
	public static void main(String[] args) throws Exception
	{
		...
		args = new String[]
		{ "", "/Users/jason/.openpnp/org.openpnp.machine.reference.vision.OpenCvVisionProvider/camera_41173395697117969.png", "/Users/jason/Desktop/result.png" };
		if (args.length > 2)
			;
		else
		{
		...
		FiducialTest.dst = Highgui.imread(args[1], 1);
		// showResult(dst);
		Imgproc.cvtColor(src, FiducialTest.dst, Imgproc.COLOR_GRAY2BGR);

		Point p = FiducialTest.fiducial(src, min, max);
		try
		{
			System.out.println("Point: " + p.x + " " + p.y);
			Highgui.imwrite(args[2], FiducialTest.dst);
			// showResult("dst",dst);
		} catch (Exception ex)
		{
			;
