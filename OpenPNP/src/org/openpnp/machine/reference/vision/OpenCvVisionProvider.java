...
public class OpenCvVisionProvider implements VisionProvider
{
	...
	static List<Point> matMaxima(Mat mat, double rangeMin, double rangeMax)
	{
		...
		for (int r = 0; r <= rEnd; r++)
		{
			...
			for (int c = 1; c <= cEnd; c++)
			{
				...
				} else if (state == MinMaxState.BEFORE_INFLECTION)
				{
					if (rangeMin <= curVal && curVal <= rangeMax)
						// MAXIMA
						if (0 < r && (mat.get(r - 1, c - 1)[0] >= curVal || mat.get(r - 1, c)[0] >= curVal))
						{
							...
							// - - -
							// - - -
						} else if (r < rEnd && (mat.get(r + 1, c - 1)[0] > curVal || mat.get(r + 1, c)[0] > curVal))
						{
							// cout << "reject:r+1 " << r << "," << c-1 <<
							// endl;
							// - - -
							// - - -
							// - x x
						} else if (1 < c && (0 < r && mat.get(r - 1, c - 2)[0] >= curVal || mat.get(r, c - 2)[0] > curVal || r < rEnd && mat.get(r + 1, c - 2)[0] > curVal))
						{
			...
			if (state == MinMaxState.BEFORE_INFLECTION)
				if (rangeMin <= curVal && curVal <= rangeMax)
					if (0 < r && (mat.get(r - 1, cEnd - 1)[0] >= curVal || mat.get(r - 1, cEnd)[0] >= curVal))
					{
						...
						// - - -
						// - - -
					} else if (r < rEnd && (mat.get(r + 1, cEnd - 1)[0] > curVal || mat.get(r + 1, cEnd)[0] > curVal))
					{
						// cout << "rejectEnd:r+1 " << r << "," << cEnd-1 <<
						// endl;
						// - - -
						// - - -
						// - x x
					} else if (1 < r && mat.get(r - 1, cEnd - 2)[0] >= curVal || mat.get(r, cEnd - 2)[0] > curVal || r < rEnd && mat.get(r + 1, cEnd - 2)[0] > curVal)
					{
	...
	{
		BufferedImage image_ = this.camera.capture();
	...
	{
		if (this.camera.getLooking() != Looking.Up)
		...
		// part to 0.
		nozzle.moveTo(this.camera.getLocation().derive(null, null, Double.NaN, 0.0), 1.0);
		// Then lower the part to the Camera's focal point in Z. Maintain the
		// part's rotation at 0.
		nozzle.moveTo(this.camera.getLocation().derive(null, null, null, Double.NaN), 1.0);
		// Grab an image.
		BufferedImage image = this.camera.settleAndCapture();
	 ...
	 * frame. Matches are returned as TemplateMatch objects which contain a
	 * Location in Camera coordinates. The results are sorted best score to
	 * worst score.
	 * 
	 * @param template
	 * @return
	...
	{
		// TODO: ROI
		BufferedImage image = this.camera.capture();
		...
		Mat debugMat = null;
		if (OpenCvVisionProvider.logger.isDebugEnabled())
		...
		for (Point point : OpenCvVisionProvider.matMaxima(resultMat, rangeMin, rangeMax))
		{
			...
			if (OpenCvVisionProvider.logger.isDebugEnabled())
			{
				Core.rectangle(debugMat, new org.opencv.core.Point(x, y), new org.opencv.core.Point(x + templateMat.cols(), y + templateMat.rows()), new Scalar(255));
				Core.putText(debugMat, "" + match.score, new org.opencv.core.Point(x + templateMat.cols(), y + templateMat.rows()), Core.FONT_HERSHEY_PLAIN, 1.0, new Scalar(255));
			}

			match.location = VisionUtils.getPixelLocation(this.camera, x + templateMat.cols() / 2, y + templateMat.rows() / 2);
			matches.add(match);
		}
		...
		this.saveDebugImage(t + "_0_template", templateMat);
		this.saveDebugImage(t + "_1_camera", imageMat);
		this.saveDebugImage(t + "_2_result", resultMat);
		this.saveDebugImage(t + "_3_debug", debugMat);

		return matches;
	}

	@Override
	public Point[] locateTemplateMatches(int roiX, int roiY, int roiWidth, int roiHeight, int coiX, int coiY, BufferedImage templateImage_) throws Exception
	{
		BufferedImage cameraImage_ = this.camera.capture();
		...
		Mat resultImage = new Mat(roiImage.cols() - templateImage.cols() + 1, roiImage.rows() - templateImage.rows() + 1, CvType.CV_32FC1);
		...
		OpenCvVisionProvider.logger.debug(String.format("locateTemplateMatches certainty %f at %f, %f", matchValue, matchLoc.x, matchLoc.y));
		this.locateTemplateMatchesDebug(roiImage, templateImage, matchLoc);

		return new Point[]
		{ new Point((int) matchLoc.x + roiX, (int) matchLoc.y + roiY) };
	}
	...
	private void locateTemplateMatchesDebug(Mat roiImage, Mat templateImage, org.opencv.core.Point matchLoc)
	{
		if (OpenCvVisionProvider.logger.isDebugEnabled())
			try
			{
				Core.rectangle(roiImage, matchLoc, new org.opencv.core.Point(matchLoc.x + templateImage.cols(), matchLoc.y + templateImage.rows()), new Scalar(0, 255, 0));
				...
				OpenCvVisionProvider.logger.debug("Debug image filename {}", file);
			} catch (Exception e)
			{
				e.printStackTrace();
			}
	...
	protected void saveDebugImage(String name, Mat mat)
	{
		if (OpenCvVisionProvider.logger.isDebugEnabled())
			try
			{
				...
				File file = Configuration.get().createResourceFile(OpenCvVisionProvider.class, name + "_", ".png");
				ImageIO.write(debugImage, "PNG", file);
			} catch (Exception e)
			{
				e.printStackTrace();
