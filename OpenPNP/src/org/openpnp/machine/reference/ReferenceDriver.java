 ...
 * until they are complete or throw an error. This Driver interface is intended
 * to model a machine with one or more Heads, and each Head having one or more
 * Nozzles and zero or more Cameras and Actuators. In OpenPnP, the Head does not
 * move on it's own. It is moved by the moving of attached objects: Nozzles,
 * Cameras, Actuators. For this reason, all movements on the driver are
 * specified as movements by one of these objects. This allows the driver to
 * make decisions as to what axes should be moved to accomplish a specific task.
 */
public interface ReferenceDriver extends WizardConfigurable, PropertySheetHolder, Closeable
{
	 ...
	 * minimum possible speed. HeadMountable object types include Nozzle, Camera
	 * and Actuator.
	 * 
	 * @param hm
	 * @param location
