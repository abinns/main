...
public class SimulatorDriver implements ReferenceDriver
{
	...
	{
		SimulatorDriver.logger.debug("actuate({}, {})", actuator, on);
		this.checkEnabled();
	...
	{
		SimulatorDriver.logger.debug("actuate({}, {})", actuator, value);
		this.checkEnabled();
		if (this.feedRateMmPerMinute > 0)
			Thread.sleep(500);
	}
	...
	private void connect()
	{
		...
		while (this.socket == null || !this.socket.isConnected())
			try
			{
				this.socket = new Socket("localhost", 9037);
	...
	{
		SimulatorDriver.logger.debug("home()");
		this.checkEnabled();
		this.send("h");
	...
	{
		SimulatorDriver.logger.debug("moveTo({}, {}, {})", new Object[]
		{ hm, location, speed });
		this.checkEnabled();
		...
		this.send(String.format(Locale.US, "m,%s,%f,%f,%f,%f", movable, location.getX(), location.getY(), location.getZ(), location.getRotation()));
	...
	{
		SimulatorDriver.logger.debug("pick({})", nozzle);
		this.checkEnabled();
		if (this.feedRateMmPerMinute > 0)
			Thread.sleep(500);
	}
	...
	{
		SimulatorDriver.logger.debug("place({})", nozzle);
		this.checkEnabled();
		if (this.feedRateMmPerMinute > 0)
			Thread.sleep(500);
	}
	...
	{
		SimulatorDriver.logger.debug("setEnabled({})", enabled);
