...
/**
 * TODO: Consider adding some type of heartbeat to the firmware. TODO: The whole
 * movement wait lock thing has to go. See if we can do a P4 type command like
 * the other drivers to wait for movement to complete. Disabled axes don't send
 * status reports, so movement wait lock never happens. Probably short moves
 * also won't.
 */
public class TinygDriver extends AbstractSerialPortDriver implements Runnable
{
	...
	public synchronized void connect() throws Exception
	{
		...
		for (int i = 0; i < 10 && !this.connected; i++)
			try
			{
				JsonObject response = this.sendCommand("{\"fv\":\"\"}", 500);
				// {"r":{"fv":0.950,"f":[1,0,10,2853]}}
				this.connectedVersion = response.get("r").getAsJsonObject().get("fv").getAsDouble();
				this.connected = true;
				response = this.sendCommand("{\"qv\":0}", 500);
				break;
			} catch (Exception e)
			{
				TinygDriver.logger.debug("Firmware version check failed", e);
			}

		if (!this.connected)
			throw new Exception(String.format("Unable to receive connection response from TinyG. Check your port and baud rate, and that you are running at least version %f of TinyG",
					TinygDriver.minimumRequiredVersion));

		if (this.connectedVersion < TinygDriver.minimumRequiredVersion)
			throw new Exception(String.format("This driver requires TinyG version %.2f or higher. You are running version %.2f", TinygDriver.minimumRequiredVersion, this.connectedVersion));

		TinygDriver.logger.debug(String.format("Connected to TinyG Version: %.2f", this.connectedVersion));
		...
		// connect.
		this.sendCommand(String.format(Locale.US, "G92 X0 Y0 Z0 A0"));
	}
	...
	public synchronized void disconnect()
	{
		...
		{
			TinygDriver.logger.error("disconnect()", e);
		}
	...
	{
		synchronized (this.movementWaitLock)
		{
			JsonObject response = this.sendCommand("G28.2 X0 Y0 Z0 A0");
			if (this.getResponseStatusCode(response) == 0)
		...
		this.sendCommand(String.format(Locale.US, "G92 X%2.2f Y%2.2f Z%2.2f A%2.2f", homeLocation.getX(), homeLocation.getY(), homeLocation.getZ(), homeLocation.getRotation()));

		this.x = homeLocation.getX();
		this.y = homeLocation.getY();
		this.z = homeLocation.getZ();
		this.c = homeLocation.getRotation();
	}
	...
	public void moveTo(ReferenceHeadMountable hm, Location location, double speed) throws Exception
	{
		...
		if (sb.length() > 0)
		{
			...
			synchronized (this.movementWaitLock)
			{
				JsonObject response = this.sendCommand("G1 " + sb.toString());
				if (this.getResponseStatusCode(response) == 0)
					this.waitForMovementComplete();
			}
		}
	...
	private void processStatusReport(JsonObject o)
	{
		if (o.has("stat"))
		{
			...
			if (stat == 3)
				synchronized (this.movementWaitLock)
				{
					this.movementWaitLock.notifyAll();
				}
		}
	}
	...
	public void run()
	{
		while (!this.disconnectRequested)
		{
			...
			{
				TinygDriver.logger.error("Read error", e);
				return;
			}
			line = line.trim();
			TinygDriver.logger.trace(line);
			try
			{
				JsonObject o = (JsonObject) this.parser.parse(line);
				if (o.has("sr"))
					// this is an async status report
					// {"sr":{"posx":0.000,"vel":7.75,"stat":3}}
					this.processStatusReport(o.get("sr").getAsJsonObject());
				else if (o.has("r"))
				{
					this.lastResponse = o;
					...
					TinygDriver.logger.error(o.toString());
				else
					TinygDriver.logger.error("Unknown JSON response: " + o);
			} catch (JsonSyntaxException e)
			{
				TinygDriver.logger.debug("Received invalid JSON syntax", e);
				// TODO: notify somehow
			}
		}
	}
	...
	public synchronized JsonObject sendCommand(String command, long timeout) throws Exception
	{
		...
		synchronized (this.commandLock)
		{
			...
			if (command != null)
			{
				TinygDriver.logger.debug("sendCommand({}, {})", command, timeout);
		...
		int responseStatusCode = this.getResponseStatusCode(response);
