...
public class NullDriver implements ReferenceDriver
{
	...
	{
		NullDriver.logger.debug("actuate({}, {})", actuator, on);
		this.checkEnabled();
		if (this.feedRateMmPerMinute > 0)
			Thread.sleep(500);
	}
	...
	{
		NullDriver.logger.debug("actuate({}, {})", actuator, value);
		this.checkEnabled();
		if (this.feedRateMmPerMinute > 0)
			Thread.sleep(500);
	}
	...
	{
		NullDriver.logger.debug("dispense({}, {}, {}, {})", new Object[]
		{ dispenser, startLocation, endLocation, dispenseTimeMilliseconds });
		this.checkEnabled();
	...
	{
		return this.feedRateMmPerMinute;
	}
	...
	{
		Location l = this.headLocations.get(head);
		if (l == null)
		{
			l = new Location(LengthUnit.Millimeters, 0, 0, 0, 0);
			this.setHeadLocation(head, l);
		}
		return l;
	}
	...
	{
		return this.getHeadLocation(hm.getHead()).add(hm.getHeadOffsets());
	}
	...
	{
		NullDriver.logger.debug("home()");
		this.checkEnabled();
		this.setHeadLocation(head, this.getHeadLocation(head).derive(0.0, 0.0, 0.0, 0.0));
	}
	...
	{
		NullDriver.logger.debug("moveTo({}, {}, {})", new Object[]
		{ hm, location, speed });
		this.checkEnabled();
		...
		Location hl = this.getHeadLocation(hm.getHead());

		if (this.feedRateMmPerMinute > 0)
			this.simulateMovement(hm, location, hl, speed);
		...
		hl = hl.derive(Double.isNaN(location.getX()) ? null : location.getX(), Double.isNaN(location.getY()) ? null : location.getY(), Double.isNaN(location.getZ()) ? null : location.getZ(),
				Double.isNaN(location.getRotation()) ? null : location.getRotation());

		this.setHeadLocation(hm.getHead(), hl);
	}

	@Override
	public void pick(ReferenceNozzle nozzle) throws Exception
	{
		NullDriver.logger.debug("pick({})", nozzle);
		this.checkEnabled();
		if (this.feedRateMmPerMinute > 0)
			Thread.sleep(500);
	}
	...
	{
		NullDriver.logger.debug("place({})", nozzle);
		this.checkEnabled();
		if (this.feedRateMmPerMinute > 0)
			Thread.sleep(500);
	}
	...
	{
		NullDriver.logger.debug("setEnabled({})", enabled);
	 ...
	 * @throws Exception
	 */
	protected void simulateMovement(ReferenceHeadMountable hm, Location location, Location hl, double speed) throws Exception
	{
		double x = hl.getX();
		double y = hl.getY();
		double z = hl.getZ();
		...
		double distancePerTick = this.feedRateMmPerMinute * speed / 60.0 / 10.0;
