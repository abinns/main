...
public class LinuxCNC implements ReferenceDriver, Runnable
{
 ...
 // connection
	...
	{
		// disconnect();
		LinuxCNC.logger.debug("connect({}, {})", serverIp, port);
		SocketAddress sa = new InetSocketAddress(serverIp, port);
		this.socket = new Socket();
		this.socket.connect(sa, LinuxCNC.CONNECT_TIMOUT * 1000);
		...
		responses = this.sendCommand("hello EMC x 1.1");
		responses.addAll(this.sendCommand("set enable EMCTOO"));

		responses.addAll(this.sendCommand("set estop off"));
		responses.addAll(this.sendCommand("set mode mdi"));
		...
		responses.addAll(this.sendCommand("set set_wait done"));
		responses.addAll(this.sendCommand("set echo off"));
		// verbose on -- all commands will be replied with ACK or NAK
		// This will be used later to determine the return status.
		responses.addAll(this.sendCommand("set verbose on"));

		this.processConnectionResponses(responses);

		if (!this.connected)
			throw new Exception("Unable to receive connection response from LinuxCNC ver 1.1. Check your server ip and port in machine.xml");
		...
		if (!this.connected)
			throw new Exception(
					String.format("Unable to receive connection response from LinuxCNC. Check your server ip and port in machine.xml and that you are running at least version %f of LinuxCNCrsh",
							LinuxCNC.minimumRequiredVersion));

		if (this.connectedVersion < LinuxCNC.minimumRequiredVersion)
			throw new Exception(String.format("This driver requires LinuxCNCrsh version %.2f or higher. You are running version %.2f", LinuxCNC.minimumRequiredVersion, this.connectedVersion));
		...
		this.sendCommand("set mdi G21");

		// Reset all axes to 0, in case the firmware was not reset on
		// connect.
		this.sendCommand("set mdi G92 X0 Y0 Z0 A0");
	}
	...
	public synchronized void disconnect()
	{
		...
		{
			LinuxCNC.logger.error("disconnect()", e);
		}
	...
	{
		this.sendCommand("set mdi G4 P0");
	}
	...
	{
		this.sendCommand("set mdi G0 Z-20"); // SafeZ
		this.sendCommand("set mdi G0 X0 Y0");
		this.sendCommand("set mdi G1 F200 Z0");
	...
	public void moveTo(ReferenceHeadMountable hm, Location location, double speed) throws Exception
	{
		...
		if (sb.length() > 0)
		{
			...
			this.sendCommand("set mdi G1 " + sb.toString());
			this.dwell();
		}
		if (!Double.isNaN(x))
			this.x = x;
	...
	{
		this.sendCommand("set mdi m3 s100");
		this.dwell();
	}
	...
	private void processConnectionResponses(List<String> responses)
	{
		for (String response : responses)
			if (response.startsWith("HELLO ACK EMCNETSVR 1.1"))
			{
				...
				this.connectedVersion = 1.1;
				this.connected = true;
				LinuxCNC.logger.debug(String.format("Connected to LinuxCNCrsh Version: %.2f", this.connectedVersion));
			}
	}
	...
	private int readChar()
	{
		...
		{
			int ch = -1;
			while (ch == -1 && !this.disconnectRequested)
				ch = this.input.read();
			return ch;
		} catch (Exception e)
		{
			LinuxCNC.logger.error("readChar()", e);
			return -1;
		}
	}
	...
	private String readLine()
	{
		...
		{
			LinuxCNC.logger.error("readLine()", e);
		}
		return null;
	}
	...
	private List<String> sendCommand(String command, long timeout) throws Exception
	{
		synchronized (this.commandLock)
		{
			if (command != null)
			{
				LinuxCNC.logger.debug("sendCommand({}, {})", command, timeout);
	...
	public void setEnabled(boolean enabled) throws Exception
	{
		...
		if (this.connected)
			this.sendCommand("set machine " + (enabled ? "on" : "off"));
	}
}
