...
// M42 - Set output on free pins, on a non pwm pin (over pin 13 on an arduino
// mega) use S255 to turn
// it on and S0 to turn it off. Use P to decide the pin (M42 P23 S255) would
// turn pin 23 on
...
// or use S<seconds> to specify an inactivity timeout, after which the steppers
// will be disabled. S0
...
// M201 - Set maximum acceleration in units/s^2 for print moves (M201 X1000
// Y1000)
// M202 - Set maximum feedrate that your machine can sustain (M203 X200 Y200
// Z300 E10000) in mm/sec
// M203 - Set temperture monitor to Sx
// M204 - Set default acceleration: S normal moves T filament only moves (M204
// S3000 T7000) in
// mm/sec^2
// M205 - advanced settings: minimum travel speed S=while printing T=travel
// only, X=maximum xy jerk,
// Z=maximum Z jerk
...
// M501 - reads parameters from EEPROM (if you need to reset them after you
// changed them
// temporarily).
...
// M601 - Show Temp jitter from Extruder (min / max value from Hotend
// Temperature while printing)
// M602 - Reset Temp jitter from Extruder (min / max val) --> Don't use it while
// Printing
// M603 - Show Free Ram
...
public class SprinterDriver extends AbstractSerialPortDriver implements Runnable
{
	...
	public void actuate(ReferenceActuator actuator, boolean on) throws Exception
	{
		...
		{
			this.sendCommand(String.format("M42 P%d S%d", this.actuatorPin, on ^ this.invertActuator ? 255 : 0));
			this.dwell();
		}
	}
	...
	public synchronized void connect() throws Exception
	{
		 ...
		 * serial port it will reset Sprinter and we'll start getting some data.
		 * On others, Sprinter may already be running and we will get nothing on
		 * connect.
		 */
		...
		if (!this.connected)
			throw new Exception(
					// String.format("Unable to receive connection response from
					// Sprinter. Check
					// your port and baud rate, and that you are running at
					// least version %f of
					// Sprinter",
		...
		// throw new Error(String.format("This driver requires Sprinter version
		// %.2f or higher. You
	...
	public synchronized void disconnect()
	{
		...
		{
			SprinterDriver.logger.error("disconnect()", e);
		}
	...
	{
		if (this.homeX || this.homeY || this.homeZ || this.homeC)
		{
			this.sendCommand(String.format("G28 %s %s %s %s", this.homeX ? "X" : "", this.homeY ? "Y" : "", this.homeZ ? "Z" : "", this.homeC ? "E" : ""));
			this.dwell();
		} else
	...
	public void moveTo(ReferenceHeadMountable hm, Location location, double speed) throws Exception
	{
		...
		if (sb.length() > 0)
		{
			sb.append(String.format(Locale.US, "F%2.4f ", this.feedRateMmPerMinute * speed));
	...
	{
		this.sendCommand(String.format("M42 P%d S%d", this.vacuumPin, this.invertVacuum ? 0 : 255));
		this.dwell();
	}
	...
	{
		this.sendCommand(String.format("M42 P%d S%d", this.vacuumPin, this.invertVacuum ? 255 : 0));
		this.dwell();
	}
	...
	private void processConnectionResponses(List<String> responses)
	{
		for (String response : responses)
			if (response.startsWith("FIRMWARE_NAME:") || response.equals("Sprinter"))
			{
				...
				this.connected = true;
				// logger.debug(String.format("Connected to Sprinter Version:
				// %.2f",
				// connectedVersion));
				SprinterDriver.logger.debug(String.format("Connected to Sprinter."));
			}
	}
	...
	public void run()
	{
		while (!this.disconnectRequested)
		{
			...
			{
				SprinterDriver.logger.error("Read error", e);
				return;
			}
			line = line.trim();
			SprinterDriver.logger.debug("< " + line);
			...
			// up.
			if (line.equals("ok") || line.startsWith("error: ") || !this.connected && line.equals("start"))
	...
	private List<String> sendCommand(String command, long timeout) throws Exception
	{
		synchronized (this.commandLock)
		{
			if (command != null)
			{
				SprinterDriver.logger.debug("> " + command);
			...
			SprinterDriver.logger.debug("Waited {} ms for command to return.", System.currentTimeMillis() - t);
		}
	...
	public void setEnabled(boolean enabled) throws Exception
	{
		...
		if (this.connected)
		{
			this.sendCommand(String.format("M84 %s", enabled ? "T" : ""));
			this.place(null);
			this.actuate(null, false);
		}
	}
}
