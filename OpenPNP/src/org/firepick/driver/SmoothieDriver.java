...
public class SmoothieDriver extends AbstractSerialPortDriver implements Runnable
{
	...
	public synchronized void connect() throws Exception
	{
		...
		this.processConnectionResponses(responses); // Flush out any start up
													 // messages
		...
		if (!this.connected)
			throw new Error(String.format(
					...
					SmoothieDriver.minimumRequiredVersion));
	...
	public synchronized void disconnect()
	{
		...
		{
			SmoothieDriver.logger.error("disconnect()", e);
		}
	...
	public void home(ReferenceHead head) throws Exception
	{
		...
		for (String response : responses)
			...
			{
				SmoothieDriver.logger.debug("echo: " + response);
				String[] coords = response.split(" ");
				this.x = Double.parseDouble(coords[2].substring(2));
				this.y = Double.parseDouble(coords[3].substring(2));
				this.z = Double.parseDouble(coords[4].substring(2));
				this.c = Double.parseDouble(coords[9].substring(2));
			}
	...
	private void processConnectionResponses(List<String> responses)
	{
		for (String response : responses)
			...
			{
				SmoothieDriver.logger.debug("echo: " + response);
				SmoothieDriver.logger.debug(String.format("Connected to Smoothie"));
			} else if (response.toUpperCase().startsWith("PROTOCOL"))
			{
				...
				{
					this.connected = true;
					SmoothieDriver.logger.debug(String.format("Connected to Smoothie with the correct firmware version"));
				} else
					SmoothieDriver.logger.debug(String.format("Wrong version of Smoothie, please obtain the latest version from https://github.com/DouglasPearless/Smoothieware"));
			}
	}
	...
	public void run()
	{
		while (!this.disconnectRequested)
		{
			...
			{
				SmoothieDriver.logger.error("Read error", e);
				return;
			}
			line = line.trim();
			SmoothieDriver.logger.debug(line);
	...
	{
		return this.sendCommand(command, 500); // changed from -1
	}
	...
	private List<String> sendCommand(String command, long timeout) throws Exception
	{
		synchronized (this.commandLock)
		{
			if (command != null)
			{
				SmoothieDriver.logger.debug("sendCommand({}, {})", command, timeout);
