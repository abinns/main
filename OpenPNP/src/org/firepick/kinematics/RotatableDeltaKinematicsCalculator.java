...
public class RotatableDeltaKinematicsCalculator
{
	...
	private double	Z_CALC_OFFSET	= (this.DELTA_Z_OFFSET - this.TOOL_OFFSET - this.DELTA_EE_OFFS) * -1;
	private double	HOME_ANGLE_X	= -67.200;																 // Angle
																											 // of
																											 // the
																											 // "X"
																											 // endstop
	...
	private double	HOME_ANGLE_Y	= -67.200;	 // Angle of the "Y" endstop
												 // sensor (0=horizontal)
	private double	HOME_ANGLE_Z	= -67.200;	 // Angle of the "Z" endstop
												 // sensor (0=horizontal)
	...
	private double PULLEY_REDUCTION = this.BIG_PULLEY_TEETH / this.SMALL_PULLEY_TEETH;

	private double XYZ_STEPS = this.XYZ_FULL_STEPS_PER_ROTATION * this.XYZ_MICROSTEPS * this.PULLEY_REDUCTION / 360.0;
	...
	public AngleTriplet calculateDelta(Location cartesianLoc) throws RotatableDeltaKinematicsException
	{
		...
		double z_with_offset = cartesianLoc.getZ() + this.Z_CALC_OFFSET; // The
																		 // delta
																		 // calc
																		 // below
																		 // places
		...
		try
		{
			solution.x = this.calculateYZ(rotated_x, rotated_y, z_with_offset);
			solution.y = this.calculateYZ(rotated_x * RotatableDeltaKinematicsCalculator.cos120 + rotated_y * RotatableDeltaKinematicsCalculator.sin120,
					rotated_y * RotatableDeltaKinematicsCalculator.cos120 - rotated_x * RotatableDeltaKinematicsCalculator.sin120, z_with_offset); // rotate
																																					 // coords
																																					 // to
			// +120 deg
			solution.z = this.calculateYZ(rotated_x * RotatableDeltaKinematicsCalculator.cos120 - rotated_y * RotatableDeltaKinematicsCalculator.sin120,
					rotated_y * RotatableDeltaKinematicsCalculator.cos120 + rotated_x * RotatableDeltaKinematicsCalculator.sin120, z_with_offset); // rotate
																																					 // coords
																																					 // to
			// -120 deg
		...
		{
			throw new RotatableDeltaKinematicsException(String.format("Delta calcInverse: Non-existing point for Cartesian location x=%.3f, y=%.3f, z=%.3f, Z_CALC_OFFSET=%.3f", cartesianLoc.getX(),
					cartesianLoc.getY(), cartesianLoc.getZ(), this.Z_CALC_OFFSET));
		}
		return solution;
	}
	...
	private double calculateYZ(double x, double y, double z) throws RotatableDeltaKinematicsException
	{
		...
		double y1 = -0.5 * 0.57735 * this.deltaF; // f/2 * tg 30
		double y0 = y - 0.5 * 0.57735 * this.deltaE; // shift center to edge
		// z = a + b*y
		double a = (x * x + y0 * y0 + z * z + this.deltaRf * this.deltaRf - this.deltaRe * this.deltaRe - y1 * y1) / (2 * z);
		double b = (y1 - y0) / z;
		// discriminant
		double d = -(a + b * y1) * (a + b * y1) + this.deltaRf * (b * b * this.deltaRf + this.deltaRf);
		if (d < 0)
 ...
 // point
	...
	{
		double t = (this.deltaF - this.deltaE) * RotatableDeltaKinematicsCalculator.tan30 / 2;
		double dtr = Math.PI / 180.0;
		double theta1 = angles.x * dtr;
		double theta2 = angles.y * dtr;
		double theta3 = angles.z * dtr;

		double y1 = -(t + this.deltaRf * Math.cos(theta1));
		double z1 = -this.deltaRf * Math.sin(theta1);

		double y2 = (t + this.deltaRf * Math.cos(theta2)) * RotatableDeltaKinematicsCalculator.sin30;
		double x2 = y2 * RotatableDeltaKinematicsCalculator.tan60;
		double z2 = -this.deltaRf * Math.sin(theta2);

		double y3 = (t + this.deltaRf * Math.cos(theta3)) * RotatableDeltaKinematicsCalculator.sin30;
		double x3 = -y3 * RotatableDeltaKinematicsCalculator.tan60;
		double z3 = -this.deltaRf * Math.sin(theta3);
		...
		double c = (b2 - y1 * dnm) * (b2 - y1 * dnm) + b1 * b1 + dnm * dnm * (z1 * z1 - this.deltaRe * this.deltaRe);
		...
		double z = -0.5 * (b + Math.sqrt(d)) / a;
		double x = (a1 * z + b1) / dnm;
		double y = (a2 * z + b2) / dnm;

		z -= this.Z_CALC_OFFSET; // NJ
	...
	{
		return new AngleTriplet(this.HOME_ANGLE_X, this.HOME_ANGLE_Y, this.HOME_ANGLE_Z);
	}

	public Location getHomePosCartesian() throws RotatableDeltaKinematicsException
	{
		return this.delta_calcForward(this.getHomePos());
	}

	// Get the raw step home positions for the three axes
	public RawStepTriplet getHomePosRaw()
	{
		return new RawStepTriplet(this.getRawStepsFromAngle(this.HOME_ANGLE_X), this.getRawStepsFromAngle(this.HOME_ANGLE_Y), this.getRawStepsFromAngle(this.HOME_ANGLE_Z));
	}

	// Get the raw steps for a specified angle
	public RawStepTriplet getRawSteps(AngleTriplet deltaCalc)
	{
		return new RawStepTriplet(this.getRawStepsFromAngle(deltaCalc.x), this.getRawStepsFromAngle(deltaCalc.y), this.getRawStepsFromAngle(deltaCalc.z));
	}

	// Return raw steps, given an angle
	public int getRawStepsFromAngle(double angle)
	{
		return (int) (angle * this.XYZ_STEPS + 0.5d);
	}
}
